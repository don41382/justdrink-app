<script lang="ts">
    ...
    let {drinkAmount = $bindable(), measureSystem = $bindable(), min, max}: {
        drinkAmount: number,
...
<div class="flex flex-col w-full h-full">
    ...
    <p class="text-secondary/80">
        Based on your age, you should drink between {VolumeConverter.to(min, measureSystem).toFixed(0).toLocaleString()}
        - {VolumeConverter.to(max, measureSystem).toFixed(0).toLocaleString()} {toVolumeName(measureSystem)}  per day.
