<script lang="ts">
    ...
    let {sipImages, selectedSipSize = $bindable(), drinkBreakMin = $bindable(), measureSystem}: { sipImages: SipImages, selectedSipSize: SipSize.Size, drinkBreakMin: number, measureSystem: MeasureSystem } = $props();

    function selectSize(sipSize: SipSize.Size) {
        selectedSipSize = sipSize
    }

    function imagePath(size: SipSize.Size) {
        switch (size) {
            case SipSize.Size.full:
                return sipImages.full
            case SipSize.Size.half:
                return sipImages.half
            case SipSize.Size.bigSip:
...
<div class="flex flex-col w-full h-full">
    <div class="flex-none flex justify-between">
        ...
        <div class="flex flex-col items-center justify-center">
            <p class="text-primary/40">{SipSize.getTextForSize(selectedSipSize) ?? "sip"} every</p>
            <p class="text-primary text-4xl">{drinkBreakMin.toFixed(0)} min</p>
        </div>
    </div>
    <div class="flex grow items-center justify-center">
        <div class="flex grow gap-x-2 justify-center">
            {#each Object.values(SipSize.sizes) as size}
                <button
