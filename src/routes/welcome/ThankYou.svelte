    ...
    <div class="w-full">
        ...
        <div class="mb-4">
            <div class="p-4 bg-secondary/50 text-white rounded-md">
                {#if licenseData.info.status === "Paid" || licenseData.payment.payment_status === "Paid"}
                    <p>Your license is active and valid.</p>
                {:else}
                    <p>Enjoy your trial for {licenseData.payment.trial_days_left} days! You can cancel your trial via Settings -> License.</p>
                {/if}
            </div>
        </div>
    </div>
