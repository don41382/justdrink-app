...
mod response {
    ...
    pub(crate) enum LicenseStatus {
        #[serde(rename(deserialize = "ACTIVE_TAIL"))]
        ActiveTrail,
    ...
    pub(super) struct Trail {
        ...
        pub(super) expired_at: DateTime<Utc>,
    }
...
impl LicenseManager {
    pub fn new(device_id: &model::device::DeviceId) -> Result<Self, anyhow::Error> {
        let client = Client::new();
        let status = Self::validate(client.clone(), device_id)?;
        Ok(Self {
            client,
    ...
    fn validate(client: Client, device_id: &model::device::DeviceId) -> Result<LicenseStatus, anyhow::Error> {
        let response =
            client
                .post(format!("https://motionminute.app/api/v1/license/validate?device={}", device_id.get_hash_hex_id()))
                .send()
        ...
        let license_status = match response.status {
            response::LicenseStatus::ActiveTrail => {
